...
import { Clear, ContentCopy, ExpandLess, ExpandMore, Search } from "@mui/icons-material";
...
export default function LogsTab({ logs, focusRequestId, onClearFocus }: Props) {
  const [query, setQuery] = useState("");
  const [expanded, setExpanded] = useState<Record<number, boolean>>({});
  const filtered = useMemo(
    () =>
      logs
        .filter((l) => (focusRequestId ? l.requestId === focusRequestId : true))
        .filter((l) => l.message.toLowerCase().includes(query.toLowerCase())),
    [logs, query, focusRequestId],
  );
      ...
      {focusRequestId && (
        <Box
          sx={{
            ...
            borderRadius: 2,
            bgcolor: (t) =>
              t.palette.mode === "dark" ? "rgba(59,130,246,0.12)" : "rgba(59,130,246,0.08)",
            border: (t) => `1px solid ${t.palette.primary.main}55`,
          }}
        >
          <Typography variant="body2" sx={{ flex: 1 }}>
            Showing logs for request{" "}
          ...
          <Tooltip title="Clear filter">
            <IconButton size="small" onClick={onClearFocus}>
